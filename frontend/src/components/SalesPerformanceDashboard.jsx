...
const SalesPerformanceDashboard = ({ salesPerson = null, isAdmin = false }) => {
    ...
    // For admin mode, don't use localStorage. For regular users, use provided salesPerson or localStorage username
    const defaultUser = isAdmin ? null : (salesPerson || localStorage.getItem('username'));
    const [currentSalesPerson, setCurrentSalesPerson] = useState(defaultUser);
    ...
    useEffect(() => {
        if (currentSalesPerson && salesUsers.length > 0) {
            const selected = salesUsers.find(u => u.username === currentSalesPerson);
            setSelectedUserName(selected ? selected.fullName : null);
        }
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setCurrentSalesPerson(username);
                                        setSelectedUser(username);
                                        // Find and set the full name of the selected user
                                        const selected = salesUsers.find(u => u.username === username);
                                        setSelectedUserName(selected ? selected.fullName : null);
                                    }}
